...
const componentSettingFields: FieldDefinition[] = [
  defineField({
    name: 'sectionId',
    title: 'Section ID',
    ...
    options: {
      source: (_, options) => {
        ...
        const { title, name, heading, _key, _type } = options.parent as ParentProps,
          typeAndKey = `${_type}-${_key}`;
...
export default componentSettingFields;
